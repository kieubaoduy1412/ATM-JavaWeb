...
public class WelcomeControllerTest {
	...
	public void getMoneyTest1() {
		...
		Mockito.when(
				welcomeDao.isEnoughMoney(customerModel.getAccountID(), amount))
				.thenReturn(true);
		TransactionModel obj = new TransactionModel();
		obj.setMessageTransaction("Your transaction was success");
		Mockito.when(
				welcomeService.getMoney(customerModel.getAccountID(), amount))
				.thenReturn(obj);
		Assert.assertEquals("Your transaction was success",
				obj.getMessageTransaction());
		Mockito.when(
				welcomeController.getMoney(amount, session, new ModelMap()))
				.thenReturn("success");
	...
	public void addMoneyTest1() {
		...
		TransactionModel obj = new TransactionModel();
		obj.setMessageTransaction("Your transaction was success");
		Mockito.when(welcomeService.addMoney(sender, accountID, amount))
				.thenReturn(obj);
		Assert.assertEquals("Your transaction was success",
				obj.getMessageTransaction());
		Mockito.when(welcomeController.addMoney(accountID, amount, session,
				new ModelMap())).thenReturn("success");
		Assert.assertEquals("success", welcomeController.addMoney(accountID,
				amount, session, new ModelMap()));
	}
	...
	public void addMoneyTest2() {
		...
		Assert.assertEquals(" Account " + accountID + " is not exist",
				obj.getMessageTransaction());
		Mockito.when(welcomeController.addMoney(accountID, amount, session,
				new ModelMap())).thenReturn("addMoney");
		Assert.assertEquals("addMoney", welcomeController.addMoney(accountID,
				amount, session, new ModelMap()));
	...
	public void addMoneyTest3() {
		...
		Assert.assertNull(customerModel.getAccountID());
		Mockito.when(welcomeController.addMoney(accountID, amount, session,
				new ModelMap())).thenReturn("login");
		Assert.assertEquals("login", welcomeController.addMoney(accountID,
				amount, session, new ModelMap()));
	}
	...
	public void sendMoneyTest1() throws ConnectException {
		...
		Mockito.when(welcomeDao.isEnoughMoney(receiverID, amount))
				.thenReturn(true);
		TransactionModel obj = new TransactionModel();
		obj.setMessageTransaction("Your transaction was success");
		Mockito.when(welcomeService.sendMoney(customerModel.getAccountID(),
				amount, receiverID)).thenReturn(obj);
		Assert.assertEquals("Your transaction was success",
				obj.getMessageTransaction());
		Mockito.when(welcomeController.sendMoney(receiverID, amount, session,
				new ModelMap())).thenReturn("success");
		Assert.assertEquals("success", welcomeController.sendMoney(receiverID,
				amount, session, new ModelMap()));
	}
	...
	public void sendMoneyTest2() throws ConnectException {
		...
		Mockito.when(welcomeService.sendMoney(customerModel.getAccountID(),
				amount, receiverID)).thenReturn(obj);
		Mockito.when(welcomeController.sendMoney(receiverID, amount, session,
				new ModelMap())).thenReturn("sendMoney");
		Assert.assertEquals("sendMoney", welcomeController.sendMoney(receiverID,
				amount, session, new ModelMap()));
	}
